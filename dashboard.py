...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE 1 — LANDING (pure Streamlit, no iframe)
# ══════════════════════════════════════════════════════════════════════════════
if st.session_state.page == "landing":
    ...
    .blob1 { width:600px; height:600px; background:rgba(99,102,241,0.15); top:-150px; left:-120px; animation-delay:0s; }
    .blob2 { width:440px; height:440px; background:rgba(139,92,246,0.12); bottom:-80px; right:-60px; animation-delay:3s; }
    .blob3 { width:320px; height:320px; background:rgba(16,185,129,0.08); top:45%; left:48%; animation-delay:6s; }
    @keyframes blobMove {
    ...
    .nav-signup {
        ...
        border: none; border-radius: 100px; padding: 8px 22px;
        text-decoration: none;
    ...
    .hero-badge {
        ...
        font-size: 0.7rem; color: rgba(165,180,252,0.9);
    ...
    .s-dot { width:7px; height:7px; border-radius:50%; flex-shrink:0; animation:blink 2s infinite; }
    .s-green { background:#10b981; box-shadow:0 0 6px #10b981; }
    .s-blue  { background:#818cf8; box-shadow:0 0 6px #818cf8; }
    .s-pink  { background:#ec4899; box-shadow:0 0 6px #ec4899; }
    .stat-sub { font-size: 0.62rem; color: rgba(255,255,255,0.28); margin-top: 4px; }
    .prog-bar { width:80px; height:3px; background:rgba(255,255,255,0.1); border-radius:2px; margin:6px auto 0; }
    .prog-fill { height:100%; border-radius:2px; background:linear-gradient(90deg,#6366f1,#a78bfa); width:99.9%; }
    ...
    .t-name { color: rgba(255,255,255,0.4); }
    ...
    .stButton > button {
        ...
        border-radius: 100px !important; padding: 15px 52px !important;
    ...
    .stButton > button:hover {
        box-shadow: 0 0 48px rgba(99,102,241,0.75) !important;
    ...
    <div class="stat-row">
        <div class="stat-card">
            ...
            <div class="stat-value"><span class="s-dot s-green"></span>Live Now</div>
            <div class="stat-sub">NYSE · NASDAQ · BSE · LSE</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Accuracy</div>
            <div class="stat-value" style="color:#a78bfa;font-size:1.5rem;">99.9%</div>
            <div class="prog-bar"><div class="prog-fill"></div></div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Companies Tracked</div>
            <div class="stat-value"><span class="s-dot s-blue"></span>100+ MNCs</div>
            <div class="stat-sub">US · India · Europe · Asia</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Data Updates</div>
            <div class="stat-value"><span class="s-dot s-pink"></span>Real-time</div>
    ...
    # Native Streamlit button — no iframe, instant click
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    .sidebar-section {
        font-size: 0.62rem; color: #3a5080; text-transform: uppercase;
        letter-spacing: 0.12em; margin: 18px 0 8px 0; padding-bottom: 5px;
    ...
    .empty-state {
        ...
        justify-content: center; padding: 100px 40px; text-align: center;
    }
    .empty-icon { font-size: 3.5rem; margin-bottom: 20px; }
    .empty-title { font-family: 'Syne', sans-serif; font-size: 1.5rem; font-weight: 700; color: #e2e8f8; margin-bottom: 12px; }
    .empty-sub { font-size: 0.9rem; color: #3a5080; line-height: 1.8; }
    .empty-hint { margin-top: 20px; padding: 12px 24px; background: rgba(99,102,241,0.08); border: 1px solid rgba(99,102,241,0.15); border-radius: 10px; font-size: 0.8rem; color: #7a90c0; }
    ...
    .section-lbl { font-size: 0.62rem; color: #3a5080; text-transform: uppercase; letter-spacing: 0.12em; margin: 16px 0 10px 0; padding-bottom: 6px; border-bottom: 1px solid rgba(99,179,255,0.07); }
    ...
    .refresh-bar { background: rgba(16,185,129,0.06); border: 1px solid rgba(16,185,129,0.15); border-radius: 8px; padding: 8px 14px; margin-bottom: 12px; font-size: 0.7rem; color: #10b981; display: flex; align-items: center; justify-content: space-between; }
    ...
    with st.sidebar:
        ...
        query = st.text_input(
            "",
            placeholder="Type company — Apple, BMW, TCS...",
        ...
        if query.strip():
            ...
            if matches:
                ...
                st.warning("No company found. Try another name.")
        ...
        period_opt = st.selectbox("", PERIOD_OPTIONS, label_visibility="collapsed")
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        run_ai = st.button("▶  Run AI Analysis", key="run_ai")

        if mode == "live" and company:
            st.markdown('<div class="sidebar-section">Live Controls</div>', unsafe_allow_html=True)
    ...
    if not company:
        ...
        <div class="empty-state">
            ...
            <div class="empty-title">Search for any company worldwide</div>
            <div class="empty-sub">
                Type a company name in the search bar on the left to get started.<br>
                Live prices · Historical charts · AI predictions · News sentiment
            </div>
            <div class="empty-hint">
                Try: Apple · Microsoft · Tesla · BMW · TCS · Infosys · Sony · Goldman Sachs
            </div>
        </div>
        """, unsafe_allow_html=True)
        st.stop()
    ...
    if mode == "live":
        ...
        <div class="refresh-bar">
            <span>🟢 Live data · Last updated: {datetime.now().strftime('%H:%M:%S')}</span>
            <span>Click 🔄 Refresh Now in sidebar to update</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with chart_col:
        guide_extra = ""
        if mode == "future":
            guide_extra = " &nbsp;|&nbsp; <span style='color:#f59e0b;'>- - -</span> Dashed amber = AI prediction"
        ...
        if mode == "future":
            st.markdown('<div class="pred-note">⚠️ AI Price Prediction — Based on historical trend. Indicative only. Not financial advice.</div>', unsafe_allow_html=True)
        ...
        if mode == "future" and pred_days > 0:
            ...
            fig.add_trace(go.Scatter(x=future_dates, y=future_pred, name=f"AI Prediction ({pred_days}d)", line=dict(color="#f59e0b", width=2.5, dash="dash"), mode="lines+markers", marker=dict(size=4, color="#f59e0b"), hovertemplate="<b>Predicted:</b> $%{y:.2f}<br>%{x}<extra></extra>"), row=1, col=1)
            fig.add_vline(x=df.index[-1], line_dash="dot", line_color="rgba(245,158,11,0.4)", annotation_text="Prediction starts →", annotation_position="top right", annotation_font=dict(size=9, color="#f59e0b"))
