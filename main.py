...
app = FastAPI(
    ...
    description="AI-powered stock analysis using LLaMA 3.1, RAG, BiLSTM and real-time data",
    version="2.0.0"
)
...
# ── AGENTIC AI TOOLS (from your original main.py) ─────────────────────────
@tool
...
def analyze_sentiment_endpoint(request: SentimentRequest):
    ...
    try:
        ...
        return {
            ...
            "confidence_score": round(result["positive_count"] / max(len(news_list), 1), 2),
...
def get_technical_indicators(request: TechnicalRequest):
    ...
    try:
        ...
        return {
            ...
            "RSI":  {"value":  result["RSI"],  "signal": result["RSI_signal"]},
...
def full_analysis(request: AnalyzeRequest):
    ...
    try:
        ...
        # Get price data for structured response
        df     = get_stock_df(request.ticker, request.period)
        latest = df.iloc[-1]
